...
use bitcoincore_rpc::{jsonrpc, Auth as RpcAuth};
use bitcoincore_rpc::{Client, RpcApi};
use jsonrpc::error::Error::Transport;
use log::{debug, info};
...
/// The JSON RPC timeout error message. It appears in [`sync_bitcoin_wallet `].
const TIMEOUT_ERROR: &str =
    "Couldn't connect to host: Didn't receive response data in time, timed out.";

/// The JSON RPC resource unavaliabe error message. It appears in [`sync_bitcoin_wallet `].
const RESOURCE_UNVALIABLE_ERROR: &str =
    "Couldn't connect to host: Resource temporarily unavailable (os error 35)";
...
impl<'a, D: BatchDatabase> DbState<'a, D> {
    ...
    fn sync_with_core(&mut self, client: &Client, is_descriptor: bool) -> Result<&mut Self, Error> {
        ...
        if is_descriptor {
            // This prevents an infinite loop caused by repeated wallet scans that fail to proceed
            // with `sync_with_core`. Ignoring the timeout allows continued synchronization,
            // avoiding the cycle where a wallet scan error leads to repeated scan attempts without
            // resuming the intended sync process.
            if let Err(err) = import_descriptors(client, start_epoch, scripts_iter) {
                match err {
                    Rpc(JsonRpc(Transport(err)))
                        if matches!(
                            err.to_string().as_str(),
                            RESOURCE_UNVALIABLE_ERROR | TIMEOUT_ERROR
                        ) => {}
                    _ => return Err(err),
                }
            }

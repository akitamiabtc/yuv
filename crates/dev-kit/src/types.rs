...
/// Fee rate strategy that is used to calculate fee for YUV transactions.
...
impl YuvTxOut {
    ...
    pub fn weight(&self) -> usize {
        ...
        (8 + bitcoin::VarInt(script_len as u64).len() + script_len) * 4
    }
    ...
    pub fn minimal_non_dust(script_pubkey: ScriptBuf, amount: u128, chroma: Chroma) -> Self {
        ...
        YuvTxOut {
            ...
            script_pubkey,
            pixel: Pixel::new(Into::<Luma>::into(amount), chroma),
        }
    }
}
...
impl Utxo {
    ...
    pub fn outpoint(&self) -> OutPoint {
        match &self {
            Utxo::Yuv(yuv) => yuv.outpoint,
        }
    }

    /// Get the `YUVTxOut` of the UTXO
    pub fn yuv_txout(&self) -> &YuvTxOut {
        match &self {
            Utxo::Yuv(yuv) => &yuv.txout,
        }
    }
}
